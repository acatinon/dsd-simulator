...
export async function getTotalCDSDBonded(ethersProvider: ethers.providers.Web3Provider): Promise<BigNumber> {
    const dsdContract = new ethers.Contract(daoAddr, daoAbi, ethersProvider);
    const totalSupply = await dsdContract.totalCDSDBonded();
